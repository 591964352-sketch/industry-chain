...
4. **(G1 预扫)列"无财报个股清单"** —— 哪些 `stock.dims6Note` 缺少"具体财报数字+[来源]"(纯🆪理由不计入)。注入后该清单会跑 G1 自检,作为双校验一部分。
不改数据、不动日期、不 commit。
```
...
【关卡 v1 挂钩】
  - **G1 覆盖率自检**：注入后自动跑 `node` 离线脚本，输出"X/Y 个股带财报、缺 N 只：清单"；缺项票的 dims6Note 开头标 "⚠️财报待核"，禁止用🆪理由顶替。
  - **G3 六维方向**：注入后输出"六维方向待确认提示"——重点扫 `valuation`(5 = 最便宜，方向翻转勿写反)+ `supply`(供不应求 → 高分)；CC 仅凭推断给的方向性分须标 "🆪待人工确认"。
逐项报告 旧值→新值+tier+来源 + "被拒/存疑清单"，双校验(node自检/全链加载/G1 覆盖率自检/抽查六维卡+擂台)，等"通过"再 commit。
```
