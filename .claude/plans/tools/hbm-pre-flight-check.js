// HBM pre-flight-check.js
// 用途：stock 上市状态预检（退市/停牌/ST/异常标注）
// 输入：动态提取 hbm.js 中所有 stock code（segments + midstream + fourQuestions）
// 输出：每只 stock 的状态报告 + 写入 .claude/scratch/hbm-stock-status-report.md
// 触发时机：每次 HBM R1 / 数据刷新任务开始前必须运行
// 教训（2026-06-21）：002288 超华科技已退市但被纳入 P1 流程，
//                   导致豆包返回大量 2026 幻觉事件并被采信为 down 判定依据。
// 复用自 pcb-pre-flight-check.js（2026-06-23 HBM R1 批 1）。
...
// 不再硬编码——避免人工维护清单与 hbm.js 实际 stock 不一致导致的"安静漏检"
const STOCK_CODES = (() => {
  const codes = new Set();
  ...
  // 2. midstream 段（HBM 先进封装/封测/模组代理——可能漏列）
...
STOCK_CODES.forEach(code => {
  ...
  if (allLocations.length === 0) {
    report.push({
      code, name: '(未找到)',
      status: '⚠️ NOT_FOUND',
      detail: '在 hbm.js 中未找到该 stock code',
...
// 控制台输出
console.log('========== HBM Pre-Flight Check · 上市状态预检报告 ==========');
console.log('检查时间: 2026-06-23');
...
// 写入报告文件
const reportContent = '# HBM Stock 上市状态预检报告\n\n' +
  '**检查时间**: 2026-06-23\n' +
...
fs.writeFileSync('d:/乌龟/产业链全景/.claude/scratch/hbm-stock-status-report.md', reportContent);
console.log();
console.log('报告已写入: .claude/scratch/hbm-stock-status-report.md');
