...
- ⑨ **G1 覆盖率自检**（数据质量关卡 v1）— 注入后跑机械自检：统计该链 `dims6` 个股中 `dims6Note` 含"具体财报数字+[来源]"的比例，**X/Y 个股带财报、缺 N 只：清单**；不到 100% 须列缺项 + 缺项票注释标 `⚠️财报待核`（详见「数据质量关卡 v1」G1）
...
## 数据质量关卡 v1（Quality Gates）

> **适用场景**：新建 / 刷新任何赛道，**判"完成"前必过 6 关**。任一关未过 → 标"草稿/待完善"，不得当完整链。
> **复盘根因**：守门只挡"坏数据"不挡"缺数据"；六维🆪主观分无方向校验；先建后审。
> **可机械自检**：G1 覆盖率 / G3 六维方向 / G6 DoD（CC 离线脚本）；G2 / G4 / G5 需人 / 联网端判定。

### G1 覆盖率关（CC 离线机械自检）

- 统计该链所有 `dims6` 个股中，`dims6Note` 含"**具体财报数字 + [来源]**"的比例
- 不到 **100%** 不算完成；**列出缺数据个股清单**，缺的票注释开头标 `⚠️财报待核`
- 禁止用 🆪 打分理由顶替财报

**合格财报注释** = ≥1 个带单位财务数（营收 / 净利 / 同比）+ 报告期 + tier。**纯 🆪 理由不计入覆盖率。**
...
- 六维属 🆪，**CC 守门拦不住方向反的分**
- 赛道级 6 个分须经**人工 / 联网端扫方向**：
  - `supply`：供不应求 → 高分；过剩 → 低分
  - `valuation`：**5 = 最便宜（方向翻转，勿写反）**
  - `durability / visibility / policy / barrier`：高分 = 好
- 每分 `reason` 须带一句现实依据（不能是空泛形容词）
- **CC 仅凭推断给的方向性分须标 `🆪待人工确认`**

### G4 顺序关（先查后注）

- 新增链 **CC 只产"结构骨架 + 待核个股清单"**，数据默认未核
- 联网端核实出注入块后再注入；**严禁先建满数据再审**
- **CC 搭完结构 ≠ 链完成**（避免"骨架已建"被当"数据已齐"）
...
1. ✅ 对齐 PCB schema（S1–S10 全过）
2. ✅ 覆盖率 100%（或缺项已标 `⚠️财报待核`）
3. ✅ 六维方向已确认（G3）
