...
| 链 | downstream 节点 | 当前 barrier | Phase A 判断依据 | 是否有系统性分析 |
|------|------|:--:|------|:--:|
| PCB | AI 服务器(40%) | high | 英伟达认证壁垒（相对明确） | ❌ 无量化 |
| PCB | 汽车电子(25%) | mid | AEC-Q 车规认证存在 | ❌ 无量化 |
| PCB | **通信/5G(20%)** | **mid** | "5G-A 升级周期"（市场增速，非壁垒指标） | ❌ 完全空白 |
| PCB | **消费电子(15%)** | **low** | "增速 0-3%·充分竞争"（市场增速+主观判断） | ❌ 完全空白 |
| 半导体设备 | 先进逻辑代工等 3 个 | extreme/high | Phase A 主观判断 | ❌ |
| 存储与接口 | 未明确设 downstream 节点 | — | — | — |
| 光模块 | 5G 电信+数据中心共 2 个 | mid/high | Phase A 主观判断 | ❌ |

**影响范围**：不限于 PCB 链——所有有 treeMap downstream 列的链（PCB/半导体设备/光模块）的 downstream 节点 barrier 均未经过系统性市场准入壁垒评估。

**待建立**（后续独立设计，不在本次 scope）：
1. 下游市场准入壁垒评估方法论——需要定义不同于个股 barrier 的评估标准（如：合格供应商数量、客户认证周期、价格竞争程度、技术路线锁定程度等）
2. 对全部链的 downstream 节点做一次全量评估
3. 将评估结果写入各链 treeMap downstream 节点的 barrier 字段
4. 此项评估会涉及 L1/L3/L4 信源（Prismark 下游市场分析、CPCA 行业报告、头部券商产业链研究），不是纯 CC 推断能完成的

**优先级 P2**：不阻塞当前四条链的完成度确认，不阻塞 push。但在启动更多新链之前，应考虑是否需要先建立统一的评估标准，避免新链的 downstream 节点继续填入无依据的主观值。

**登记 commit**：本次（7.18 · treeMap barrier 批次修正同步登记）
