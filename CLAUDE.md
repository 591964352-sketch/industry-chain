...
### §11.10.1 P0 批次·第一阶段完成 + 第二阶段待启动(2026-07-05 commit 6.37 立)

> **重要:P0 批次明确分为两个阶段,不要笼统标记"P0 已完成"**

#### 第一阶段(本次完成):reason 字段补全

**7 条 reason 字段已写入 pcb.manual.js**(2026-07-05 按 4 类风险分级处理后版本):
...
**001389/durability 未写入**——auto-mode 拒绝执行(理由:豆包 response 含"unverified doubao responses"风险),本字段待下批次重新设计豆包 prompt 后再处理。

#### 第二阶段(待启动):score 统一校准

**关键认知:第一阶段 ≠ 完成 P0**

- 7 条 reason 字段补全后,**score 数字全部维持原值**:
  - 600183 visibility 5/605589 visibility 5/002938 durability 5/002938 visibility 5/301200 visibility 5 → **理论上应下修至 4**(豆包初判 + §10 严格判定),但因无 L1 公告支撑,本批次**不能下修**
...
为防止豆包 hallucination 数字直接写入 pcb.manual.js(违反 §6.7.1/§6.7.2 防御),本批次新增"高风险数字完整梳理"环节,系统化列出 7 条 reason 中所有"具体家数/具体百分比/具体认证日期/具体供应商名称"类数字,并按 4 类风险分级处理:
...
**这 2 处幻觉已登记到 §6.7.1/§6.7.2 案例归档区**(见下面 commit 6.37 新增的 2 项案例)。
...
- ✅ 7 条 reason 字段写入 pcb.manual.js
- ✅ §11.10.1 P0 状态登记(2 阶段区分)
- ✅ 2 处豆包 hallucination 登记到 §6.7.1/§6.7.2 案例归档区
- ✅ 待启动:第二阶段 score 校准 + 001389 durability 重写
