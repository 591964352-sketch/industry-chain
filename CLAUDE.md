...
#### §6.8.4 「先审阅后 commit」铁律（2026-07-17 commit 7.44 后立 · 永久生效）

> **触发事故**：commit 7.43（SI 链 valuation.note 改写）和 7.44（SE 链 valuation.note 改写）在执行过程中，CC 因脚本调试多轮耗时，急于推进下一批次，在用户尚未完成审阅确认的情况下直接执行了 `git add` + `git commit`。事后审阅发现 SE 链中微/北华创/拓荆三只卡口开头残留了冗余括号文本 `（当前仅含 abstract_ths L1 财务基本面...）`——该括号与新开头句内容完全重复且暴露了 `abstract_ths` 开发者工具名。虽通过 `git commit --amend` 修正，但流程已经倒置。

**核心规则**：任何由用户显式要求"审阅确认后再 commit"的改动，必须满足以下条件后才允许执行 `git add` 和 `git commit`：

1. **用户必须已看到实际的、逐字的修改后内容**（不得用摘要、统计数字或代表性样本代替）
2. **用户必须已发出显式的确认信号**（如"确认通过""没问题""可以 commit"等）
3. **上述两个条件缺一不可**——脚本执行成功、语法检查通过、数据验证零错误，都不能替代用户审阅确认

**禁止行为**：

| 反模式 | 后果 |
|--------|------|
| ❌ 脚本执行完成后立即 git add + git commit | 跳过了用户的审阅窗口 |
| ❌ 用"XX 处全部替换成功，验证通过"替代完整内容的逐字段展示 | 用户无法发现冗余括号、措辞生硬等非脚本可检测的问题 |
| ❌ 因"调试耗时太长"而压缩审阅环节 | 本次事故的直接根因——调试疲劳导致流程急躁 |
| ❌ 试图事后用 `git commit --amend` 补救 | amend 能修数据但不能修流程——已经损害了审阅信任 |

**完整执行序列**（涉及用户审阅确认的改动）：

```
1. 脚本/Edit 执行 → 完成数据修改
2. node --check 语法验证 → 通过
3. 提取修改后的完整实际内容 → 贴出给用户审阅（逐字段、逐股票、不要摘要）
4. 【等待用户显式确认】← 这是不可跳过的阻塞点
5. git add + git commit
6. git show HEAD 核实 → 贴出给用户确认
```

**事故案例归档**（1 例）：
- **2026-07-17 commit 7.43/7.44 · SI+SE 链 valuation.note 跳步 commit**：CC 在用户尚未看到 SI 5 只 + SE 4 只实际修改后全文的情况下直接 commit。事后审阅发现 SE 链 3 处冗余括号。通过 amend 修正。根因：脚本 unicode 匹配问题导致多轮调试，CC 疲劳后急于推进。

**复用方式**：本规则适用于所有涉及用户可见字段的批量修改。数据处理类脚本（如 refresh_fundamentals.py）的纯数据刷新不受此限——但如果刷新结果会影响用户看到的文本内容（如 dims6 reason 重写），同样需要遵守本条。

```

#### §6.8.5 Object.keys() 全字段枚举铁律（2026-07-18 commit 7.47 后立 · 永久生效）

> **触发事故**：commit 7.41 清理 semicon-equip 链 fourQuestions 的 §11.23 编号时，脚本只硬编码了 `q2note`/`q3note`/`q4note` 三个字段名，遗漏了 `q1note`。24 处 §11.23 残留暴露了超过 24 小时未被发现，直到用户截图指出。commit 7.47 修复时用 `Object.keys()` 枚举了全部 12 个字段逐一检查，确认 q1note 是唯一遗漏位置。

**核心规则**：任何涉及"扫描/检查/修改某个数据结构中多个字段"的任务，**必须先做一次 `Object.keys()` 全字段枚举**，生成完整字段清单，再决定对哪些字段执行操作。禁止凭印象挑选"认为重要"的字段。

**执行序列**（强制）：

```
1. Object.keys(targetObject) → 输出完整字段清单
2. 逐字段确认：是否需要处理？处理方式？
3. 对照清单编写处理逻辑，确保每个"需要处理"的字段都有对应的代码路径
4. 处理完成后，再次用 Object.keys() 验证目标对象中所有字段的最终状态
```
...
| 反模式 | 后果 | 案例 |
|--------|------|------|
| ❌ 凭印象写 `['q2note','q3note','q4note']` | 遗漏 q1note 等未被想起的字段 | commit 7.41 → 7.47 补漏 |
| ❌ 只检查"看起来重要的"字段 | 遗漏看似不重要但同样暴露内部编号的字段 | — |
| ❌ 用 `for (var k in obj)` 后仍手工挑选字段 | 遍历了但未逐字段处理，效果等同于硬编码 | — |

**适用范围**：所有数据扫描/检查/批量修改任务。不限于 fourQuestions——segments、chokePoints、supplyGap、treeMap、chainStory 等任何嵌套结构中的字段操作均适用。

**事故案例归档**（1 例）：
- **2026-07-18 commit 7.47 · q1note 遗漏**：commit 7.41 脚本硬编码 `q2note`/`q3note`/`q4note`，遗漏 `q1note`（24 处 §11.23 残留）。根因：未做全字段枚举，凭印象挑选"数据不足"模式的字段。本次 q1note 是唯一遗漏字段，其余 11 个字段确认干净。

**复用方式**：后续任何链的字段清理/扫描任务，第一步必须是 `Object.keys()` dump 完整字段清单。
...
> **§10.1 visibility 打分铁律（2026-07-16 光模块链排查立·永久生效）**：visibility 严格衡量**信息渠道层级**（L1公告/L4券商/L5媒体/无可查渠道），**不衡量信息内容的正面/负面**。公司业绩差≠visibility 低——亏损公司照样有 L1 季报+L4 券商覆盖→vis 应 ≥3。业绩好坏通过 durability 和 valuation 反映，不要让 visibility 承担双重判断任务。§6.15 亏损规则影响 valuation/durability，不影响 visibility。
>
> **常见违规**：❌"连续亏损→可见度差→vis=2" ❌"营收下滑→vis=2" ❌"Q1暴跌→vis=2" ✅"小市值尾部厂商→经查证无L4覆盖+无L1订单→vis=2"
>
> **事故案例**（2026-07-16）：光模块链烽火/特发/三安 vis 2→3，联特 vis 2→3；storage-interface 链景嘉微/纳芯微/裕太微/芯原 vis=2 待修正。
...
### §10.4 物理卡口类型分类（chokePointType · 2026-07-17 立 · 通用标准）

> **触发**：先进封装链 Phase B 完成时，鼎龙股份被判定为"准卡口 ★★☆"——其单一产品线均非全球寡头（PSPI 全球 HD Micro 主导 >70% / CMP 抛光垫 Dow/Cabot 主导 CR2>60% / 临时键合胶 Brewer Science 主导 >70%），但三条线同时布局的国产供应商仅鼎龙一家。现有的 `global-scarcity`（全球稀缺型）和 `domestic-substitution`（国产替代型）均无法准确描述这种"多线广度壁垒"的性质。本节建立三种 chokePoint 类型的正式定义和适用边界。

#### 类型一：`global-scarcity`（全球稀缺型）

| 项目 | 内容 |
|------|------|
| **定义** | 单一产品/技术/环节的全球供给高度集中（≤3 家可批量供货），构成不可替代的物理卡口 |
| **硬指标** | 全球 ≤3 家可量产 + 认证壁垒 ≥18 月（或行业公认等效） |
| **典型特征** | 寡头垄断格局明确（单一产品维度），新进入者即使有资金也难以在 3 年内突破 |
| **数据来源** | L3 行业报告（SEMI/Yole/Prismark）的全球供应商名录 + L4 券商竞争格局分析 |
| **chokePoint 强度** | ★★★（命中 4/4 或接近满分·核心物理卡口） |
| **已有案例** | 芯源微·临时键合/解键合（全球仅 TEL/EVG/芯源微三家）· 深南电路·FC-BGA 载板（全球 ≤5 家量产·大陆唯一） |

#### 类型二：`domestic-substitution`（国产替代型）

| 项目 | 内容 |
|------|------|
| **定义** | 单一产品/技术/环节的全球供给不构成寡头垄断（>3 家），但在国内供应链中该产品被海外厂商高度主导，国产替代进程中有且仅有一家国内公司取得实质性突破（从 0 到 1） |
| **硬指标** | 全球 >3 家量产 + 国产化率 <20% + 国内仅 1-2 家可量产 |
| **典型特征** | 国产替代的"唯一先行者"——非全球寡头，但国内追赶路径清晰，先发优势构成 2-5 年的战术窗口期 |
| **数据来源** | L1 年报（国产替代进展披露）+ L3 行业报告（全球格局）+ L4 券商国产替代深度研究 |
| **chokePoint 强度** | ★★☆（命中 3/4·潜在卡口）或 ★★★（国产替代路径极清晰且战略意义极高） |
| **已有案例** | 光模块链·中际旭创（光模块制造·全球 10+家·但国内出货量第一·市占 25-30%） |

#### 类型三：`domestic-platform`（国产平台型壁垒 · 2026-07-17 立）

| 项目 | 内容 |
|------|------|
| **定义** | 单一产品线均非全球寡头（每条线全球 >3 家），但多条关键产品线同时布局的广度构成国产唯一的平台型壁垒——国内没有第二家公司同时覆盖这组产品线，客户可实现一站式采购，形成研发协同 + 渠道共享 + 客户粘性的复合护城河 |
| **硬指标** | 同时覆盖 ≥3 条关键产品线 + 每条线的国产供应商 ≤2 家 + 国内唯一同时覆盖全组产品线的公司 |
| **典型特征** | "广度壁垒"而非"深度壁垒"——单一维度的竞争力不足以构成卡口，但多维度的同时覆盖创造出独特的协同优势。性质：商业效率壁垒（客户一站式采购 + 研发协同 + 渠道共享）而非技术独占壁垒（不是"只有我会做"而是"只有我同时都会做"） |
| **数据来源** | L1 年报（各产品线营收披露）+ L3 行业报告（各产品线的全球格局）+ L4 券商平台型公司分析 |
| **chokePoint 强度** | ★★☆（准卡口·命中 3/4）——不构成 ★★★ 因为单线非全球寡头·存在被逐个突破的理论风险 |
| **已有案例** | 鼎龙股份·PSPI+CMP 抛光垫+临时键合胶三线国产唯一（单线均非全球 ≤3 家·PSPI 全球 HD Micro 主导 >70% / CMP 抛光垫 Dow/Cabot CR2>60% / 临时键合胶 Brewer Science >70%·但三线同时布局的国产供应商仅鼎龙一家） |

#### 三种类型的区分矩阵

| 判定维度 | global-scarcity | domestic-substitution | domestic-platform |
|------|:--:|:--:|:--:|
| 单线全球寡头（≤3家）？ | ✅ 是 | ❌ 否（>3家） | ❌ 否（>3家） |
| 国内唯一/独家？ | ✅ 是（通常也是国内唯一） | ⚠️ 可能是（仅1-2家国产） | ✅ 是（多线同时覆盖的唯一国产） |
| 壁垒性质 | 技术独占·物理瓶颈 | 先发优势·战术窗口 | 广度协同·商业效率 |
| chokePoint 强度 | ★★★ | ★★☆ 或 ★★★ | ★★☆ |
| 被突破的理论风险 | 极低（物理/工艺壁垒） | 中（海外巨头降价+国内追赶者） | 中偏高（任一单线可能被国内竞争者逐个突破·但三线同时突破概率极低） |

#### 判定流程（新增 chokePoint 时必须走）

1. **先判定单线**：该公司的核心产品/技术在全球维度是否 ≤3 家可量产？→ 若是 → `global-scarcity`
2. **若不满足 ≤3 家**：该公司是否在国产替代中处于唯一/绝对领先位置（国内仅 1-2 家）？→ 若是 → 继续判定 ③
3. **区分广度 vs 深度**：该公司的优势来自"单一产品线的国产替代深度"还是"多产品线同时覆盖的广度"？
   - 单线深度 → `domestic-substitution`
   - 多线广度（≥3 条关键线同时覆盖·国内唯一）→ `domestic-platform`
4. **如果既不满足单线深度也非多线广度** → 不构成正式 chokePoint

**违反本节 = chokePoint 类型判定无依据/混淆类型边界 → 违反 §6.8 数据准确度优先原则。**

### §10.5 业务纯度三档分类（businessPurity · 2026-07-21 立 · 通用标准）

> **触发**：雅克科技（002409）在半导体材料链 seg3（电子特气）中，电子特气营收仅占 4.85%，但公司半导体材料业务整体（前驱体 SOD+光刻胶+电子特气）占 52.12%。这个问题暴露了 `businessPurity` 字段的度量口径缺陷——应衡量 **chain 级纯度**（该公司是否真的是这条产业链里的公司）而非 **segment 级纯度**（该公司在这个具体品类里占比多大）。

#### 三档定义

| 档位 | 条件 | 渲染标签 | 含义 |
|------|------|:--:|------|
| **pure-play** | chain 域业务占公司总营收 > 50%（默认不显式标注） | （无标签） | 公司主业就是这条产业链——纯正标的 |
| **mixed** | chain 域业务占 10-50%，或同源技术自然延伸但实际营收占比小 | `⚠️ 纯度偏杂`（琥珀色·`#ca8a04`） | 产业链内但不是纯正标的——业务真实但被其他主业稀释 |
| **tangential** | chain 域业务 < 10%，或仅有远期概念无实质营收 | `⚠️ 业务纯度低`（红色·`var(--color-muted)`） | 整条产业链对公司来说是边角料 |

#### 核心原则：chain 级纯度决定 businessPurity，segment 级纯度作为补充细节

**度量口径 = chain 域整体**（该公司在这条产业链的**全部业务**合计占比），**不是**该公司在当前 segment 分类下的单一品类占比。

| 测量层级 | 回答的问题 | 用于 |
|------|------|------|
| **chain 级** | "这家公司真的是这条产业链里的公司吗？" | `businessPurity` 字段标注 |
| **segment 级** | "在这个具体品类里，公司的占比多大？" | `fundamentals.note` 字段的颗粒度信息 |

**判断决策树**：

1. **统计 chain 域全部业务营收占比**：该公司所有归属于此产业链的业务线合计占总营收的百分比
2. **> 50% → `pure-play`**（或省略字段·默认）
3. **10-50% → `mixed`**：即使是核心技术同源延伸（如精测电子前道量检测→光模块测试·占比 15-25%），只要 chain 域业务真实存在且有一定体量，就标 `mixed`
4. **< 10% → `tangential`**：chain 域业务是边角料
5. **segment 级细节写入 `fundamentals.note`**：如 "电子特气仅占 4.85%·整体半导体材料业务占 52.12%"

#### 典型案例对照（验证框架一致性）

| 股票 | chain | chain 域业务 | chain 级占比 | segment 级占比 | 标注 | 判断逻辑 |
|------|------|------|:--:|:--:|:--:|------|
| 菲利华 300395 | PCB | Q布 | 4.88% | 4.88% | `tangential` | chain 级 ≈ segment 级·都是边角料 |
| 南大光电 300346 | 半导体材料 | MO源（主要下游LED/化合物半导体·非前道晶圆制造）+光刻胶 | ~10%（含争议·MO源下游与本链聚焦有偏差） | 光刻胶 <1% | `tangential` | 即便宽松计数也勉强 10%·与 chain 聚焦领域偏差大 |
| 永鼎股份 600105 | 光模块·光互联 | 光通信（鼎芯光电） | ~20% | 光芯片更小 | `mixed` | chain 域达实质性业务线门槛 |
| TCL中环 002129 | 半导体材料 | 半导体硅片 | <20% | <20% | `mixed` | chain 域真实业务·被光伏稀释 |
| 晶瑞电材 300655 | 半导体材料 | 光刻胶+超纯化学品 | ~12% | 光刻胶 ~12% | `mixed` | chain 域真实但体量小 |
| **雅克科技 002409** | **半导体材料** | **前驱体SOD+光刻胶+电子特气** | **52.12%** | **电子特气 4.85%** | **`mixed`**（本次判断·2026-07-21） | **chain 级超 50% 本应 pure-play，但电子特气 segment 极小而跨链 C 类复用背景特殊——整体半导体材料是主业、本链 seg3 只是其中一个次要分支** |

> **雅克科技为何不标 `pure-play`**：52.12% 客观上超 50% 门槛，但它是 C 类跨链复用标的（主场在 storage-interface 链，本链仅 seg3 电子特气一个 segment 引用），且电子特气仅 4.85%。若标 `pure-play` 会向读者传递"电子特气=雅克核心主业"的错误信号。`mixed` + fundamentals.note 精确拆解，是最诚实的标注。

#### 渲染规则

- `tangential` → 红色标签 `⚠️ 业务纯度低`（`color:var(--color-muted);background:rgba(246,70,93,0.08);border:rgba(246,70,93,0.2)`）
- `mixed` → 琥珀色标签 `⚠️ 纯度偏杂`（`color:#ca8a04;background:rgba(234,179,8,0.08);border:rgba(234,179,8,0.2)`）
- `pure-play` 或字段不存在 → 不渲染任何标签
- 两个颜色语义分明：红色 = "公司不在这条链里"（彻底边角料），琥珀色 = "公司在链里但纯度不高"（被其他业务稀释）

**违反本节 = businessPurity 标注的口径选择错误（segment 级代替 chain 级·或反之）→ 违反 §6.8 数据准确度优先原则。**
...
### §11.16 格林达 chokePoint 候选待核实(2026-07-21 commit · 半导体材料链 Phase B 第四批立)

> **触发**：Phase B 第四批(seg4 湿电子化学品)豆包查询中，格林达(603931)被初步评估为 chokePoint 候选(domestic-substitution ★★☆)，核心依据是"国内唯一 G5 级 TMAH 供应商"。但豆包重制报告的【6. 未查到】栏诚实列出"国内其他实现 G5 级 TMAH 研发/量产企业完整清单"——"国内唯一"这个绝对化断言并未被本次查询的任何信源(L1/L3/L4)证实，而是延续了 Phase A 骨架阶段被列入黑名单的旧描述。本批次审慎降级为**"待核实候选"**，不正式认定 chokePoint。

**当前可确认的事实(有信源支撑)**：

| 事实 | 信源层级 | 信源 |
|------|:--:|------|
| 格林达已实现 G5 级 TMAH 量产 | L2+L3 | SEMI 化学品分级标准 + 中国电子材料行业协会 2026 报告 |
| TMAH 显影液国产化率 <20% | L3 | 中国电子材料行业协会 2026 年 7 月细分国产化率数据 |
| 客户认证周期 12-18 月 | L4 | 东北证券行业框架研报(行业通用逻辑) |
| **格林达是"国内唯一"G5 级 TMAH 供应商** | **❌ 未被证实** | **豆包诚实归入【6. 未查到】·无 L1/L3/L4 信源** |

**待核实条件(触发 chokePoint 正式认定的前置要求)**：

1. 从中国电子材料行业协会获取国内 G5 级 TMAH 供应商完整名录(L3·行业协会数据)
2. 或从头部晶圆厂(中芯国际/华虹/长江存储/长鑫存储)的供应商信息披露中交叉验证
3. 如证实国内仅格林达 1 家 → 正式认定 `domestic-substitution ★★☆`
4. 如证伪(国内 ≥2 家) → 降级为"国内领先者之一"，不构成 chokePoint，barrier reason 进一步修正

**当前处置**：
- chokePoint 候选池仍为 seg1-3 的 3 个(沪硅产业/彤程新材/南大光电)·seg4 不新增
- 格林达 barrier reason 使用"国内领先者"表述(不含"唯一"字样)·末尾显式标注"暂不定级 chokePoint(等待证实/证伪国内唯一 G5 级 TMAH 供应商·见 §11.16)"
- 格林达 trendNote/position/barrier reason 三个字段含此次修正的全部上下文

**核实优先级 P2**：格林达 chokePoint 认定依赖 L3 行业协会数据(非实时行情数据)，不阻塞当前半导体材料链 Phase B 推进。可在 seg5 完成后、跨 segment 对比定级前统一核实。

**涉及文件**：`data/semiconductor-materials.manual.js` 格林达 stock 块(dims6[5] barrier reason + position + trendNote)

**登记 commit**：本次(半导体材料链 Phase B 第四批)

---

### §11.16a check_schema_completeness.js 规则扩展导致的系统性 baseline errors(2026-07-21 发现 · 待统一评估)

> **触发**：半导体材料链 Phase B 第四批写入前，`check_schema_completeness.js` 报告 16 errors（`TOP.demandChainMeta: MISSING` ×1 + `SEG[0-4].barrier/choke/globalLandscape: MISSING` ×15）。通过 `git stash` + 重跑检查对比确认：**这 16 errors 在 Phase A 骨架建立时就已存在，非本批次引入。**

**问题性质**：`check_schema_completeness.js` 的检测规则在后续迭代中扩展后，新增了对 `demandChainMeta`、`seg[i].barrier`、`seg[i].choke`、`seg[i].globalLandscape` 等字段的存在性检查。这些字段在 PCB 链中存在，但在半导体材料链（以及其他多条新链）的 Phase A 设计阶段并未纳入 scope。检查脚本升级后，这些存量字段缺失被报告为 error。

**影响范围（全链普遍性）**：

| 字段 | 缺失的链 | 是否为设计意图 |
|------|------|:--:|
| `TOP.demandChainMeta` | semicon-equip / optical-module / storage-interface / semiconductor-materials / advanced-packaging / low-altitude / … | ✅ PCB 专属需求链可视化模块·其他链设计上不包含 |
| `SEG[i].barrier` | semiconductor-materials（5 segments 全缺） | ⚠️ 标识段位级别 barrier 的片头标签·PCB 有·新链 Phase A 未纳入 |
| `SEG[i].choke` | semiconductor-materials（5 segments 全缺） | ⚠️ 标识段位是否有 chokePoint·PCB 有·新链未纳入 |
| `SEG[i].globalLandscape` | semiconductor-materials + advanced-packaging（多条链全缺） | ⚠️ 全球竞争格局简述·PCB 有·新链未纳入 |

**当前处置**：暂不处理（非数据缺陷·是检测工具规则演进与存量数据之间的 gap）。本待办登记为已知 baseline，后续 commit 时不再重复排查这 16 项。

**后续统一评估（P3 · 不阻塞任何链推进）**：
1. 方案 A：补齐所有新链的这些字段（工作量较大·需逐条评估每个 segment 的 barrier/choke/globalLandscape）
2. 方案 B：调整 `check_schema_completeness.js`，将这些字段标记为 `optional` 或按链豁免（如 `demandChainMeta` 仅 PCB 需要）
3. 推荐方案 B——因为这些字段中至少 `demandChainMeta` 明确是 PCB 专属功能，不应作为全链强制检查项

**违反本节**：无（仅为检测工具演进 vs 存量数据 gap 的登记·不构成数据缺陷）。
...
### §11.16b 第三批 seg3 注入脚本缺陷·dims6 数据从未写入事故(2026-07-22 发现并修复 · 🔴 严重)

> **事故等级**：🔴🔴 严重——commit message 声称的 moat=56/56/46/74 四个值中两个存在计算错误(华特应为 60/雅克应为 78)，且全部 4 只股票的 dims6 reason 字段从未写入过文件（一直保留 Phase A 占位符 `"(Phase B补)"`），moatScore/timingScore/quadrant 全部为 null。该状态从 Phase B 第三批 commit(973926f·2026-07-17)持续至 2026-07-22 全链排查才发现，历时约 5 天。

**事故发现过程**：

第四批 seg4 完成后做全链 moatScore null 值排查时，通过 `node -e` 代码遍历发现金宏气体/华特气体/凯美特气/雅克科技四只的 moatScore 全部为 null。进一步 `git stash` + 逐 commit 追溯确认：这四只的 moatScore **从 Phase A 骨架建立之初就一直是 null**，第三批注入脚本只写了 position/trend/fundamentals 字段，完全漏写了 dims6 的 score/reason/tier 和 moatScore/timingScore/quadrant。

**事故全链路**：

1. **Phase A**（9c4ae5b）：骨架建立，四只 stock 的 dims6 填入占位估计值（scores 恰好与最终审阅值不同），reason 全部为 `"(Phase B补)"`，tier 全部为 `"estimate"`，moatScore 全部为 null
2. **Batch 1-2**（9843e63/a2c6c83）：seg1-2 的注入脚本正确写入了 dims6 reason 和 moatScore。这是正确的——说明代码逻辑本身没问题，是第三批的特定脚本存在缺陷
3. **Batch 3**（973926f）：注入脚本存在 bug——**只写了 position/trend/trendNote/fundamentals，完全漏写了 dims6 和 moatScore**。分析讨论中计算出的 moat=56/56/46/74 从未进入文件。且分析环节自身存在两处算术错误（华特加权和正确值 3.00 误算为 2.80→moat 应为 60 非 56；雅克加权和正确值 3.90 误算为 3.70→moat 应为 78 非 74）
4. **验证失败**：第三批审阅时贴出了计算文本并获得批准，但没有人用 `node -e` 实际读取文件确认写入——等同于"批准了一份从未被执行的操作"
5. **Batch 4**（e1d0165）：第四批正常执行，但验证时也未发现第三批的遗留 null 值
6. **2026-07-22 发现**：全链 `node -e` 遍历暴露 4 处 null，`git stash` 对比确认非本批次引入，逐 commit 追溯确认根因
7. **首次修复失败**：用全局字符串替换写入 moatScore 时触发了 §11.19 完全相同的多块命中 bug（`"moatScore": null` 匹配了其他 stocks），被即时发现并回滚
8. **二次修复成功**：改用 stock-code 精确限界替换（每只 stock 独立 block 内替换），正确写入全部 4 只

**最终修复后正确值**（基于审阅通过的 dims6 scores + 代码公式精确计算）：

| Stock | moatScore | timingScore | quadrant | 备注 |
|-------|:--:|:--:|------|------|
| 688106 金宏气体 | 56 | 40 | skip-borderline | moat=56 < 60·在边界(55-64) |
| 688268 华特气体 | 60 | 40 | hold-borderline | moat=60 ≥ 60·在边界(55-64)·原分析误算为 56 |
| 002549 凯美特气 | 46 | 40 | skip | moat=46 < 60·不在边界 |
| 002409 雅克科技 | 78 | 60 | core | priority→core 映射·原分析误算为 74·与 SI 主场链 moat=78 一致 |

**防止重犯的强化规则（新增 · 永久生效）**：

1. **每次 commit 前必须用 `node -e` 实际读取文件验证写入内容**——不只是验证语法正确（`require` 不报错），而是**逐字段验证关键值不是 null/占位符**
   ```bash
   # 强制验证模板——每次批量注入后执行
   node -e "global.window={};require('./data/<chainId>.manual.js');var M=global.window['<NS>_MANUAL'];['<code1>','<code2>'].forEach(function(c){var s=M.stocks[c];if(s.moatScore===null||s.dims6[0].reason==='(Phase B补)')throw new Error(c+' INCOMPLETE');});console.log('ALL OK')"
   ```
2. **"贴出计算文本供审阅"不能替代"确认写入"**——审阅环节批准的是分析内容，不等于数据已经进入文件。审阅确认后必须多一步：脚本执行 → 读取文件 → 打印实际值 → 与审阅确认的值对照
3. **dim6 reason 字段的非空检查纳入 commit 前必须项**——任何 reason 包含 `"(Phase B补)"` 或 `"(待补)"` 的 stock 在声称"本批次完成"之前必须被视为未完成
4. **任何涉及 `str.replace()` 的批量写入脚本，必须在写入前做唯一性检查**（§11.19 规则已于本次事故的首次修复失败中再次验证其必要性）

**涉及文件**：`data/semiconductor-materials.manual.js`（688106/688268/002549/002409 四个 stock 块）+ `scripts/__inject_sm_batch3_fix.js`（修复脚本）+ `scripts/__inject_sm_batch3_dims6_fix.json`（数据源）

**修复 commit**：待 commit（2026-07-22）
...
### §11.28 下游市场准入壁垒评估维度（2026-07-16 立 · 项目系统性空白 · P2）

> **触发**：treeMap barrier 一致性扫描（commit 7.16+7.18）发现 PCB 链 2 处 downstream 节点 barrier 偏差（通信/5G=mid、消费电子=low），但判定"下游市场节点不应套用公司最高真实 barrier 规则"。进一步核实确认：这两个 barrier 值来自 Phase A 建骨架时的主观判断，note 字段虽然引用了 Prismark 市场份额数据，但**市场份额 ≠ 市场准入壁垒**，从未做过系统性的"该下游市场对 PCB 供应商的准入壁垒"分析。**这不是"已豁免=已解决"，而是项目中的系统性空白。**

**问题定性**：treeMap downstream 列的节点代表**下游终端市场**（AI 服务器/汽车电子/通信/消费电子），衡量的是"PCB 供应商进入该市场的难度"（认证周期、客户集中度、供应商数量、产品同质化程度、切换成本等）。这与 midstream/materials/equipment/sideBranches 列衡量"该产业链环节内公司的竞争壁垒"是**两个完全不同的维度**。

**当前状态（四条链 13 个 downstream 节点全量排查）**：

**A 类：barrier 已标注但均为 Phase A 主观判断，无系统分析支撑（5 个）**：

| 链 | 节点 | 当前 barrier | 依据（note 字段） | 问题 |
|------|------|:--:|------|------|
| PCB | 汽车电子(25%) | mid | "车规 PCB+连接器·电动化催化(CPCA 2025)" | CPCA 引用的是市场规模，不是准入壁垒 |
| PCB | 通信/5G(20%) | mid | "5G-A 升级周期(Prismark 2026)" | Prismark 引用的是市场周期，不是准入壁垒 |
| PCB | 消费电子(15%) | low | "增速 0-3%·充分竞争(Prismark 2026)" | "充分竞争"是主观判断，无量化支撑 |
| 半导体设备 | 功率/模拟/MEMS | high | "SiC/GaN 扩产潮(SEMI 2025)" | SEMI 引用的是扩产趋势，不是准入壁垒 |
| 光模块 | 5G 电信(CPO/光模块) | mid | 无 note（仅 companies 列表） | 完全无依据 |

**B 类：barrier 完全未标注（4 个）**：

| 链 | 节点 | 当前 barrier |
|------|------|:--:|
| 存储与接口 | AI 服务器/HPC(英伟达 B200/GB300) | **—** |
| 存储与接口 | 数据中心/云计算(内存池化) | **—** |
| 存储与接口 | 企业存储/内存模组 | **—** |
| 光模块 | 数据中心·企业级光网络应用 | **—** |

**C 类：barrier 可能碰巧正确但同样未经系统评估（4 个·纳入全量复核）**：

| 链 | 节点 | 当前 barrier | 碰巧可能合理的理由 |
|------|------|:--:|------|
| PCB | AI 服务器(40%) | high | 英伟达 GPU 生态高度集中，认证壁垒客观上确实高 |
| 半导体设备 | 先进逻辑代工(7-28nm) | extreme | 全球先进逻辑代工仅台积电/三星/Intel 三家 |
| 半导体设备 | 存储 IDM(DRAM/NAND) | extreme | DRAM 三寡头+NAND 五家，格局清晰 |
| 光模块 | AI 数据中心(800G/1.6T) | extreme | 英伟达 Blackwell/Rubin 平台锁定效应 |

**执行计划**：

| 批次 | 范围 | 节点数 | 预估工作量 |
|:--:|------|:--:|:--:|
| 第一批 | A 类（有 barrier 标注但无依据） | 5 个 | 3-4 小时 |
| 第二批 | B 类（barrier 完全空白） | 4 个 | 2-3 小时 |
| 复核 | C 类（碰巧可能正确） | 4 个 | 1-2 小时 |
| **合计** | — | **13 个** | **6-8 小时** |

**方法论**：见 §15 下游市场准入壁垒五维评估框架（2026-07-16 立）。

**优先级 P2**：不阻塞当前四条链 push。建议在启动下一条新链时，将下游壁垒五维纳入 Phase A 初始建立流程，避免新链重蹈"先填占位值后修正"的覆辙。

**登记 commit**：7.18（treeMap barrier 批次修正同步登记）+ 7.19（本批次更新为完整 A/B/C 分类清单）

---

### §11.29 chokepointType 字段渲染层零引用·P1 待办（2026-07-17 立）

> **触发**：check_schema_completeness.js v2 全链扫描发现 PCB 10 条 warning，其中 5 条为 `chokepointType` 缺失。深入排查后发现更严重的问题：**`chokepointType` 字段在 `index.html` 约 351K 字符的渲染代码中引用次数为 0**——§10.4 建立的三种卡口类型分类体系（global-scarcity/domestic-substitution/domestic-platform）已在数据层认真填写，但从**未被渲染层展示给读者**。

**问题定性**：这是"数据层已做出实质性判断但从未传达给读者"的信息断层。现状——页面上芯源微（全球≤3家·物理稀缺）和深南电路（全球≤5家·大陆唯一·但真瓶颈在ABF膜）和鼎龙股份（三线广度·国产唯一平台）在页面上呈现完全相同的视觉效果："🔒 ★★★"标签。读者无从判断这几只卡口的性质有什么本质区别——这与 §10.4 花多轮讨论建立的"不能让读者误解卡口性质"原则直接冲突。

**当前数据覆盖情况**（2026-07-17 全链排查）：

| 链 | chokePoint 总数 | 有 type | 缺 type | 使用字段名 | 类型体系 |
|------|:--:|:--:|:--:|------|------|
| PCB | 10 | 5 | 5 | `chokepointType` | 旧名（physical/alpha-competitive） |
| 半导体设备 | 5 | 0 | 5 | — | 全缺 |
| 存储与接口 | 5 | 5 | 0 | `chokepointType` | 旧名（physical/alpha-competitive） |
| 光模块 | 6 | 6 | 0 | `chokePointType` | 新名（global-scarcity/domestic-substitution） |
| 先进封装 | 3 | 3 | 0 | `chokePointType` | 新名（global-scarcity/domestic-platform） |

**字段名不一致**：PCB/semicon-equip/storage-interface 使用 `chokepointType`（全小写 p），光模块/先进封装使用 `chokePointType`（驼峰大写 P）。**下次实施时先统一字段名**——全部五链统一为 `chokePointType`（驼峰大写·与 §10.4 标准命名一致），**禁止让渲染代码去兼容两套不同的字段名逻辑**。统一工作在"第一步"数据层补全时同步执行，不等到渲染实现时再处理。

**渲染方案设计**（待实施）：

在 chokePoint 卡片的 `🔒 segment` 行新增彩色 tag，根据 type 值渲染不同类型标签：

| chokepointType | 标签文案 | 建议颜色 | 投资含义 |
|------|------|------|------|
| `global-scarcity` | 🌍 全球稀缺型 | 金色 `var(--accent)` | "全球≤3家·供给侧物理稀缺" |
| `domestic-substitution` | 🇨🇳 国产替代型 | 蓝色 `var(--blue)` | "国产替代先行者·战术窗口期" |
| `domestic-platform` | 🧩 国产平台型 | 青色 `#0d9488` | "多线广度·国产唯一平台" |

**执行顺序（强制·不可跳步）**：

> ⚠️ **必须先做第 1、2 两步的数据清理，才能做第 3 步的渲染实现。禁止跳过数据清理直接写渲染代码——否则渲染代码需要兼容两套字段名+三套类型体系（新旧混杂），会导致渲染逻辑过度复杂且容易出错。**

**第 1 步**（字段名统一·数据层）：将全部五链的字段名统一为 `chokePointType`（驼峰大写 P·与 §10.4 标准命名一致）。PCB/semicon-equip/storage-interface 的 `chokepointType`→重命名为 `chokePointType`。

**第 2 步**（类型体系升级·数据层）：
- PCB 前 5 条 + 存储与接口 5 条：旧类型名升级（physical→global-scarcity, alpha-competitive→domestic-substitution）
- PCB 后 5 条：补充 `chokePointType` 字段（扩容时遗漏），按各自卡口性质判定类型
- 半导体设备 5 条：从零补充 `chokePointType` 字段（此前完全缺失），按设备卡口性质判定
- 光模块 6 条 + 先进封装 3 条：已使用 §10.4 新体系，无需变更

**第 3 步**（渲染层·新增展示·依赖第 1、2 步完成）：在 index.html 的 chokePoint 卡片渲染代码中，`🔒 ${c.segment}` 行新增彩色 type tag。此时五链的字段名和类型体系已全部统一，渲染代码只需读取 `c.chokePointType` 一个字段、按 §10.4 三类型映射标签颜色即可。

**优先级 P1**：不是锦上添花的视觉优化，而是数据层已完成的实质性判断从未传达给读者的信息断层——与"不能让读者误解卡口性质"原则直接冲突。

**不阻塞当前 push**：P1 而非 P0，因为当前五链的页面核心功能（六维打分/四问筛选/供需缺口/树状图/赛道概览）均已验证正常。此待办为渲染层的增强功能，可以在下次会话优先处理。

**登记 commit**：7.31（check_schema_completeness.js v2 全链扫描触发）

---

### §11.30 semicon-equip 链 methodologyNotes 中的 `null` 字面量显示瑕疵（2026-07-17 立 · P3）

> **触发**：光模块链 fullGuide 上线前全链回归扫描时发现 semicon-equip 链页面 `innerText` 含 1 处 `null`。Playwright DOM 定位确认其在用户可见区域。

**具体位置**：`#section-meta > DIV.section-body > DIV.card > OL > LI`（第 7 个 LI 条目）。父级 LI 以"**并非所有产业链环节都存在物理卡口**：🔩 **半导体设备重构骨架说明 (Phase A - 2026-07-09)**"开头。

**实际文本**：`meta.ltFit = null(对齐 PCB)`

**性质**：methodologyNotes 文本中硬编码的 JavaScript 字面量 `null`，而非数据缺失导致的 `undefined` 渲染异常。`ltFit` 在数据层确实是 `null`（表示"尚未计算"，与 PCB 模板一致，是正确的默认值），但 methodology 文本原样输出了 JS 字面量，普通读者无法理解。

**影响**：用户可见，不会导致页面布局破损或功能异常，但会让不熟悉编程的投资者困惑。严重度远低于之前的 `undefined` 事故（那是数据缺失导致 JS 求值后原样渲染）。

**建议修复**：将 methodologyNotes 中的 `null` 改为中文表述 `（待计算）` 或 `（暂无）`。同步检查其他 4 条链的 methodologyNotes 是否有同类硬编码 `null`/`undefined` 字面量。

**优先级 P3**：不阻塞任何功能，下次修改 semicon-equip.manual.js 或做 methodologyNotes 统一审计时顺带修复。

**登记 commit**：7.40（本次）

---

### §11.31 光模块链 methodologyNotes 零散内部术语残留（2026-07-19 立 · P2）

> **触发**：全链 methodologyNotes 审计发现光模块链 460 字说明中存在零散内部术语，虽不构成完整开发者 changelog（与 semicon-equip 旧的 1963 字相比），但有 4 处需要改写：

1. `双层架构(参照 semicon-equip P0 经验,避免返工)` — "P0" 是项目阶段编号
2. `方案 B 决定,用户 review 通过` — "review"是开发工作流用语
3. `类似 PCB 链 sideBranches` — 跨文件引用 + 英文字段名
4. `参照 PCB 链 sideBranches 只做工艺说明不占实际持股 的处理方式` — 同上

**待办**：光模块链 methodologyNotes 整体重写（参照 PCB 链 264 字简洁标准），本批次仅清理了最明显的开发阶段标记（`stage 2 commit 6.70 立` + `commit message 显式说明`），其余术语留待重写时一并处理。

**优先级 P2**：不影响可读性但降低了专业感。下次统一审计时可连同 semicon-equip 重写经验一并处理。

**登记 commit**：本次

---

### §11.32 光模块链 methodologyNotes 整体书写风格待重写（2026-07-19 立 · P2）

> **触发**：§11.31 的 3 处具体开发者术语已清理完毕，但整段文字用中点符号（`·`）串联各种技术要点的书写方式（如"unique 去重 41 只""双层架构""seg3 保留为空段 + sub-card only 工艺说明"），读起来仍然像内部技术备忘录，未达到 PCB 链 264 字那种真正流畅、面向投资者的方法论说明的标准。

**当前问题**（非术语暴露·纯风格层面）：
- 整段使用 `·` 作为分隔符串联 10+ 个技术要点，缺乏段落结构和逻辑连接词
- 保留了"unique 去重""valuation tier 全部降级""sub-card only"等偏工程化的表述习惯
- 没有像 PCB 链那样用连贯的自然段落向读者解释"为什么这条链的数据是这么组织的"

**待办**：参照 PCB 链 264 字范例，将光模块链 methodologyNotes 重写为面向投资者的流畅段落实说明。

**优先级 P2**：不影响数据准确性和可读性，但降低了专业感和用户友好度。下次统一审计时一并处理。

**登记 commit**：本次

---

### §11.33 先进封装链沃格光电 qXpSrc 字段 §6.15 残留（2026-07-19 立 · P3）

> **触发**：全五链四问字段 Object.keys() 枚举复核时发现先进封装链沃格光电（603773）的 `q1pSrc`/`q2pSrc`/`q4pSrc` 三个数据来源标注字段中引用 `§6.15`（亏损公司专项规则编号）。

**性质**：`qXpSrc` 是 variant 四问中的信源标注字段，**四问筛选表格不渲染**（渲染只读取 `qXpClaim`），用户不可见。属于纯数据清洁任务。

**待办**：将 `§6.15` 替换为中文规则名"亏损公司专项规则"，下次编辑 advanced-packaging.js 时顺带处理。

**优先级 P3**：不影响用户可见内容，纯数据层编号残留。
...
| **对照文档人工填写与磁盘数据不同步** | CC 提交 PCB/semicon-equip 验收对照文档（供用户浏览器核对）时，从记忆或前轮报告手写字段值，未从磁盘 `data/*.js` 读取。结果：commit 6.80 之后 barrier/moat 大量变化（拓荆 5→4、微导纳米 5→3、长川科技 moat 67→79、先导基电 66→82 等），但文档未同步——用户拿错误文档核对页面反而误判页面显示有问题。事故规模：47 处笔误（含 2 处 chokePoint 核心数据错位） | semicon-equip 对照文档「6.5 segment 结构调整」章节（2026-07-12 手写·基于 commit 6.80 之前快照·未读磁盘实际数据）→ 修复：scripts/generate_audit_doc.js 程序化生成 | 🔴 **任何 PCB/semicon-equip/未来新链的验收对照文档，必须程序化生成（直接读磁盘 `data/*.js` + `data/*.manual.js`），禁止人工从记忆或旧报告抄写字段值**。正确做法：① 提供 `scripts/generate_audit_doc.js` 作为唯一对照文档生成工具 ② 输出文件存入 `.claude/scratch/<chainId>_audit_doc.md` ③ 每次 PCB/semicon-equip 有重大更新后（commit 后）重新跑一次脚本 ④ 用户拿到的对照文档末尾必须有"程序化生成自 `scripts/generate_audit_doc.js`"标识用于追溯 |
...
### §13.9 四维健康度定期扫描制度（2026-07-16 立 · 永久生效）

> **触发**：本轮会话中 storage-interface 链的业务错位污染、光模块链的 treeMap 渲染 bug、PCB 链的四问筛选 variant 断层——这几次严重问题都不是系统性排查发现的，而是被偶然点开某个板块才发现的。在 push 之前必须做一次真正主动的、覆盖所有已知问题类型的全维度扫描。

**适用时机**（任一触发）：
- 任何链 push 到 GitHub Pages 之前
- 任何链 Phase B+ 批量数据注入完成后
- 任何链 segments/treeMap/fourQuestions 结构调整后
- 每两周一次的全链定期巡检

**四项检测（必须全部执行，不可跳过任何一项）**：

| # | 检测项 | 工具 | 检测内容 |
|:--:|------|------|------|
| **①** | 业务身份一致性 | `node scripts/check_business_identity_match.js <chainId>` | 公司名/代码是否错位、是否出现跨链污染术语 |
| **②** | treeMap sourceSegment 粒度陷阱 | `node scripts/check_tree_sourceseg.js <chainId>` | 是否存在同一 sourceSegment 被多节点共享+空 companies 的模式 A 高危 |
| **③** | treeMap 节点 barrier 一致性 | `node scripts/check_treemap_barrier.js <chainId>` | 节点 barrier 等级与节点内公司真实 dims6.barrier 最高值是否一致 |
| **④** | fourQuestions hits 一致性 | `node scripts/check_fourq_hits.js <chainId>` | hits 字段是否等于 q1-q4（或 variant q1p-q4p）中 true 的数量 |

**全链批量执行**（push 前一键跑通）：
```bash
# 四项检测对全部链批量运行
for chain in pcb semicon-equip storage-interface optical-module; do
  echo "=== $chain ==="
  node scripts/check_business_identity_match.js $chain
  node scripts/check_tree_sourceseg.js $chain
  node scripts/check_treemap_barrier.js $chain
  node scripts/check_fourq_hits.js $chain
  echo ""
done
```

**判定规则**：
- ① 的报警需要**逐项人工判定**是"已知误报"还是"新发现的真实问题"——不能批量标注"均为预存"
- ② 模式 A（高危）→ 必须修复；模式 B（一般）→ 登记待办
- ③ 偏差 1 档 → 登记待办评估；偏差 ≥2 档 → 本次必须修复或给出合理解释
  - **下游市场节点例外**（2026-07-16 立）：treeMap downstream 列的节点代表**下游市场**（如通信/5G、消费电子），其 barrier 应衡量"该市场的供应商进入壁垒"，而非"节点内公司各自的壁垒最高值"。下游节点内既包含终端客户公司又包含 PCB 制造商，两类公司的 barrier 含义不同，**不应套用"节点 barrier = 公司最高真实 barrier"规则**。此类偏差标记为"下游市场节点·不适用"，不计入待修复清单。
  - **`check_treemap_barrier.js` 当前未内置下游节点识别逻辑**，对 downstream 列的偏差需要人工判定豁免。
- ④ 任何不一致 → 本次必须修复

**扫描报告归档**：每次全链扫描结果存入 `.claude/scratch/health-scan-<date>.md`，包含四链×四维完整矩阵。

**假阴性防御铁律（2026-07-16 立 · 永久生效）**：

任何检测脚本如果输出"无法检测/无数据/跳过/不适用"这类结论，**必须先核实这个结论本身是否成立**，不能直接采信为"跳过是合理的"。

> **事故案例（2026-07-16）**：`check_treemap_barrier.js` v1 对 storage-interface 和 optical-module 输出"无 manual 层，无法检测"——但这两个链的 manual.js 文件一直存在且包含完整的 Phase B 六维数据。根因是脚本 `getManual()` 函数硬编码了 PCB_MANUAL 和 SEMICON_EQUIP_MANUAL 两个 namespace，同时 `allFiles` 的 filter 条件排除了 `.manual` 文件。光模块链 InP 衬底节点 barrier 偏高 1 档的真实问题因此被漏诊。**假阴性比假阳性危险**——假阳性会发现去复核，假阴性会让人误以为"跳过是正常的"而永不复查。

**防御规则**：
1. 所有检测脚本的 namespace 获取**必须使用泛型模式**（`chainId.toUpperCase().replace(/-/g, '_') + '_MANUAL'`），**禁止硬编码特定链的 namespace**
2. 脚本输出"无数据"类结论前，必须交叉验证数据文件确实不存在（`fs.existsSync` + 文件内容非空 + namespace 已注册），不能仅凭"查找失败"就断定"数据不存在"
3. 新脚本上线时必须用 ≥2 条已知有完整数据的链做真阳性验证，确认脚本能正确检测到已知问题

**违反本节 = 在未完成四维健康度扫描的情况下 push → 视为违反 §6.8 数据准确度优先原则。**
...
## §15 下游市场准入壁垒五维评估框架（通用 · 所有产业链 · 2026-07-16 立）

> **触发**：treeMap barrier 一致性扫描发现 PCB 链 downstream 节点 barrier 是 Phase A 历史遗留未验证值，金量排查确认四条链 13 个 downstream 节点中 9 个需要系统性评估——这是**项目级系统性空白**。downstream 节点衡量的是"供应商进入该下游市场的难度"，与 midstream/materials/equipment 节点衡量"产业链环节内公司的竞争壁垒"是两套完全不同的方法论。

### §15.1 适用范围

**评估对象**：treeMap downstream 列的**下游终端市场节点**。这些节点代表产业链产品最终流向的终端市场（如 AI 服务器、汽车电子、消费电子、通信/5G、晶圆代工、数据中心等）。

**核心问题**：新供应商进入这个下游市场有多难？

**与 dims6 个股产品壁垒的关系**：

| 对比维度 | dims6 个股产品壁垒 | §15 下游市场准入壁垒 |
|------|------|------|
| 评估对象 | 某公司在某产业链环节的竞争壁垒 | 某下游市场对供应商的整体进入难度 |
| 评估粒度 | 个股 × 6 维 | 市场节点 × 5 维 |
| 直接回答的问题 | "这家公司有多难被替代？" | "进入这个市场有多难？" |
| 调用场景 | stock 筛选、chokePoint 认定、moat 评分 | treeMap downstream 节点 barrier 标注 |

**共享底层数据**：两套体系独立并行但共享同一批 L1/L3/L4 信源——从同一份 Prismark/券商研报/下游客户年报中提取不同维度的指标。dims6 关注"公司的竞争位置"，下游壁垒关注"市场的进入难度"。

### §15.2 五维评估指标

#### ① 客户认证壁垒（Certification Barrier）

新供应商进入该下游市场所需的产品认证周期、认证成本、客户审核严格程度。

| 分数 | 标准 | 典型场景 |
|:--:|------|------|
| **5** | 认证≥18月 + 需客户联合开发 + 单次认证成本>5000万 | 英伟达 GPU 平台 PCB 认证、台积电设备供应商认证 |
| **4** | 认证 6-18 月 + 多轮样品验证 + 单次成本 1000-5000 万 | 车规 AEC-Q 认证、数据中心光模块认证 |
| **3** | 认证 3-6 月 + 标准产品检测 + 单次成本 100-1000 万 | 工业设备 PCB、企业级服务器 |
| **2** | 基础认证<3 月 + 单次成本<100 万 | 消费电子配件、通用 PCB |
| **1** | 无认证要求 + 现货采购 | 标准消费品 PCB、低端 FPC |

**数据来源**：L1 下游客户年报（供应商管理制度披露）+ L4 券商行业深度（"行业壁垒"章节）+ L3 行业标准文档（AEC-Q/ISO/TS 16949 等）。

#### ② 在位者锁定深度（Incumbent Lock-in）

该下游市场已有供应商的客户关系深度——长期协议覆盖率、联合开发绑定程度、切换成本。

| 分数 | 标准 | 典型场景 |
|:--:|------|------|
| **5** | 前 3 大供应商锁定≥80% 份额 + 客户切换成本极高（产线停产风险） | 台积电与 ASML EUV 光刻机独家供应关系 |
| **4** | 前 3 大占 50-80% + 切换需重新走完整认证流程 | 英伟达 PCB 供应商体系、苹果 FPC 供应链 |
| **3** | 前 5 大占 30-50% + 年度议价 + 有替代方案 | 汽车 Tier1 PCB 供应商 |
| **2** | 供应商分散 + 切换成本低 + 多源采购 | 通用服务器 PCB |
| **1** | 完全竞争 + 纯价格驱动 + 随时切换 | 低端消费电子 PCB |

**数据来源**：L1 下游客户年报（前五大供应商集中度披露）+ L3 行业报告（供应商份额分析）+ L4 券商产业链研究。

#### ③ 合格供应商池规模（Qualified Supplier Pool）

全球/国内范围内，能够满足该下游市场质量/技术/产能要求的合格供应商数量。

| 分数 | 标准 | 典型场景 |
|:--:|------|------|
| **5** | 全球 ≤3 家合格 | EUV 光刻机（仅 ASML）、HBM 堆叠键合设备 |
| **4** | 全球 4-6 家 | AI 服务器 PCB（沪电/胜宏/鹏鼎/深南/广合等） |
| **3** | 全球 7-15 家 | 800G 光模块（中际/新易盛/Coherent/光迅/剑桥等） |
| **2** | 全球 16-50 家 | 汽车 PCB |
| **1** | >50 家 + 充分竞争 | 消费电子 PCB、通用 FPC |

**数据来源**：L3 行业报告（供应商名录+份额排名）+ L4 券商竞争格局分析 + L1 下游客户供应商名单。

#### ④ 技术路线锁定程度（Technology Roadmap Lock-in）

下游客户的技术平台是否深度绑定特定供应商的方案——私有协议 vs 标准接口、平台生命周期、升级路径对供应商的依赖程度。

| 分数 | 标准 | 典型场景 |
|:--:|------|------|
| **5** | 私有协议 + 单供应商绑定 + 平台生命周期≥5年 + 升级路径依赖原厂 | NVIDIA NVLink 交换机 PCB（生益电子+沪电）、苹果 iPhone FPC |
| **4** | 行业标准但深度定制 + 2-3 家可替换 + 平台周期 3-5 年 | 英伟达 GPU 服务器 PCB、CPO 光引擎 |
| **3** | 行业标准 + 多供应商 + 年度迭代 | 通用服务器 PCB、数据中心光模块 |
| **2** | 标准化产品 + 多平台兼容 | 消费电子 PCB、通信基站 PCB |
| **1** | 完全通用 + 无平台锁定 | 低端单/双面板 |

**数据来源**：L3 技术路线图（Prismark/LightCounting/Yole/SEMI）+ L4 券商技术分析 + L1 下游客户技术路线披露。

#### ⑤ 市场增长与利润吸引力（Market Attractiveness）

该下游市场的增速和利润水平是否吸引新供应商涌入。**这是反向指标**——高增长+高利润会吸引新进入者，侵蚀在位者壁垒；低增长+薄利润让新进入者望而却步，在位者壁垒反而稳固。

| 分数 | 标准 | 说明 |
|:--:|------|------|
| **5** | 低增速(<5% CAGR) + 薄利润(<15% 毛利率) | 新进入者无动力，在位者壁垒稳固 |
| **4** | 中等增速(5-10%) + 合理利润(15-25%) | 有限吸引力 |
| **3** | 高增速(10-20%) + 较高利润(25-35%) | 有吸引力，新进入者开始出现 |
| **2** | 爆发增长(20-30%) + 高利润(35-45%) | 大量新进入者涌入 |
| **1** | 超级增长(>30%) + 暴利(>45%) | 新进入者蜂拥，在位者壁垒被快速侵蚀 |

**数据来源**：L3 市场预测（Prismark/IDC/Gartner CAGR）+ L1 下游客户年报（毛利率水平）+ L4 券商市场展望。

### §15.3 综合评定与 barrier 映射

五个维度评分后取**加权综合**（①②③④ 各 25%，⑤ 为修正项），映射到 treeMap node barrier：

| 综合分范围 | barrier | 说明 |
|:--:|------|------|
| 4.0-5.0 | extreme | 极高市场准入壁垒 |
| 3.0-3.9 | high | 高壁垒 |
| 2.0-2.9 | mid | 中等壁垒 |
| 1.0-1.9 | low | 低壁垒 |

**与 dims6 的关键差异**：dim6 六维取**等权平均**（moat 计算），下游壁垒取**加权综合**——因为维度①认证 + ②在位者是市场准入的核心障碍，维度③④是结构性特征，维度⑤是动态修正因子。

### §15.4 新链 Phase A 建立时的强制集成（铁律 · 2026-07-16 立）

> **本节为 §15 的最高优先级规则。任何新链在 Phase A 骨架建立阶段，必须同步完成 downstream 节点的五维评估。**

**强制步骤**（新链 Phase A 骨架建立流程的必经环节）：

1. **downstream 节点定义时**：每个节点必须同时填写五个维度的 score + reason（reason 字段记录评分依据和信源，格式等同于 dim6 reason）
2. **barrier 取值**：node barrier 必须从五维综合评分映射得出，**禁止填入 Phase A 主观判断值**（如"这个市场看起来比较难进所以标 high"）
3. **note 字段**：必须区分"市场规模/增速数据"和"准入壁垒分析"两类引用——引用 Prismark/SEMI 的市场规模数据只能放在 note 里作为背景，**壁垒评分依据必须是独立的供应商格局/认证周期/锁定程度分析**
4. **L1/L3/L4 信源引用**：至少三个维度有明确的信源引用（至少 1 条 L3 行业报告 + 2 条 L4 券商分析），前两个维度（认证+在位者）优先使用 L1 下游客户年报的实际数据
5. **无数据的诚实标注**：如果某个维度确实无法从公开渠道获取数据（如新兴市场缺乏 L3 报告），标注"数据不足/待核实" + tier='estimate'——**不得编造数字填充**

**验收标准**：新链 Phase A 提交时，§13.5 人工确认清单必须包含第 11 项——"downstream 节点五维评估：每个节点有 5 维 score+reason≥2 维有明确 L3/L4 信源引用"。此项失败 → 不准上线。

**违反本节 = 新链 downstream 节点填入未经验证的 Phase A 主观判断值 → 违反 §6.2 红线（未核实标注）+ §6.8 数据准确度优先原则违反。**

### §15.5 现有链 downstream 节点待办（§11.28）

四条已完成链 13 个 downstream 节点的全量排查结果和 A/B/C 三级分类清单见 §11.28。执行优先级 P2（长期任务·不阻塞当前 push），预计全量完成 6-8 小时。建议在启动下一条新链之前完成至少 A 类 5 个节点的评估，以确保后续新链能参照已验证的范例。
