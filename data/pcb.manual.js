...
(function(MANUAL){
  ...
  MANUAL.stocks = {
    ...
    '002463': { code:'002463', name:'沪电股份', rank:1, barrier:'极高', tier:'primary',
      ...
      dims6:[{key:'durability',score:5,trend:'up',tier:'L1',reason:'AI 算力结构性上行+GB300/Rubin 持续放量;沪电为英伟达 H100/H200 提供 22-26 层高多层板,H200 UBB 主板份额超 70%(截至 2025),泰国工厂 2025.10 投产承接 GB300 订单,延续性极强 → 5',verifiedAt:'2026-06-23'},{key:'visibility',score:5,trend:'up',tier:'L1',reason:'26Q1 营收 62.14 亿(+53.91% 同比),归母 12.42 亿(+62.9% 同比),英伟达份额>50%;2025 净利 38.22 亿(+47.74% 同比);AI 营收占比 Q1 季报口径 ~60%、全年口径 15.9%(嵌套口径:AI 占 PCB 业务,公司主营 PCB 占总营收 95.77%);业绩兑现极强,趋势向上 → 5',verifiedAt:'2026-07-02'},{key:'policy',score:3,trend:'flat',tier:'L2',reason:'AI capex 市场驱动+IC 载板 02 专项+大基金二期关联,政策中性偏顺风;制造端政策驱动有限,趋势走平 → 3',verifiedAt:'2026-06-23'},{key:'supply',score:2,trend:'down',tier:'L3',reason:'整赛道 AI PCB 供给略过剩,但 78 层 M9 细分赛道 2025 年全球供给缺口 18%(沪电 62%/欣兴 38%,全球仅 2 家量产) · 公司 M9 利用率 92%,2026Q4/2027Q3 分两期扩产 5.8 万㎡/月 · Prismark《2025-2026 高端高多层算力 PCB 专项报告》+IHS Markit《全球高端 AI 硬件 PCB 供需预测》双源确认 · 保留 78 层 M9 全球独家/62% 份额与 GB200/GB300 全系认证细节 → 2',verifiedAt:'2026-07-02'},{key:'valuation',score:2,trend:'down',tier:'L1',reason:'本次 baostock L1 实测(2026-07-08 收盘,sz.002463,5 年 daily 1210 个交易日 rank):PE-TTM 57.35 倍 / 5y 历史分位 97.77% + PB(MRQ) 14.69 倍 / 5y 历史分位 97.77% + PS-TTM 11.68 倍 / 5y 历史分位 97.77%(各窗口:5y 97.77% / 3y 96.28% / 2y 94.43% / 1y 88.89% / 6mo 77.69% / 3mo 57.14%)。原数据 asOf 2026-06-16 已过时,本次刷新至 2026-07-08。依据 §10 valuation 5 档表:5 分 PE/PB/PS 分位 <30%;4 分 30-50%;3 分 50-70%;2 分 70-85%;1 分 >85% 或历史极高位。本次 baostock L1 实证 PE 5y 分位 97.77% + PB 5y 分位 97.77% + PS 5y 分位 97.77%(三档全部远超 85% 极高位档,取最严档):严格按 §10 表应判定 1 分档(估值历史极高位)。但现有 score=2 与 §10 理论值 1 存在 +1 档偏差(差距 30pp+)。处理:按用户原话本次不修改 score/trend,维持 score=2 / trend=down / tier=L1,reason 显式标注 §11.9 实测偏差,等用户明确授权后再按 §11.9 校准流程处置 ▍▍▍▍**§11.9 偏差标注(等用户授权后处置)**:实测三档分位均超 85% 与现有 score=2 不符,差 1 档。可能原因:① 卡口逻辑强(沪电 002463 rank=1 barrier=极高)对冲估值高位;② L4 头部券商研报支持的估值溢价;③ 估值口径需结合卡口逻辑综合评估。本次仅刷新实测数字,不重写评分判断(等用户明确授权)。▍▍tier 字段特殊说明:本字段核心信源 L1 baostock 实测(财务时序 + PE/PB/PS 5y 历史分位)+ L1 一季报 baostock 实测(净利+营收);严格取 L1 反映可核验度,不算 L3/L4。▍豆包自查清单·高风险栏:不采用 §6.7.1 经验推断(避免豆包 逻辑推导 伪装真实拉取),所有 PE/PB/PS 数字采用 baostock L1 实测值;具体同业 PE 对比归【6. 未查到】(akshare 申万行业 PB 中位数接口未实测);无虚构接口引用。无 hallucination 内容。 ｜来源:L1 baostock 实测(sz.002463 2026-07-08,5y daily 1210 个交易日 PE/PB/PS 历史分位)+ L1 沪电股份 2025 年报(L1 primary)+ L1 沪电股份 2026 一季报(L1 primary)+ position/investableReason 字段(estimate·本公司基本面事实陈述)+ segments idx=6 PCB 中游(估值口径定位)',verifiedAt:'2026-07-08'},{key:'barrier',score:5,trend:'flat',tier:'L1',reason:'78 层 M9 全球独家量产（broker 华泰 2026-05-25）+GB200/GB300 全系认证+AI 板良率 92-98%+全球仅沪电与日本 Ibiden 通过认证(后者未量产),壁垒极高;豆包 2026-06-26 确认 → 5',verifiedAt:'2026-06-26'}],
