...
输出：每个 stock 加 4 个字段 · 写回 pcb.auto.js
  · pePercentile (3年窗口·当前 pe_ttm 在历史中的百分位·0-100·commit 4.54 默认 5y→3y)
...
# ★ commit 4.54：PE 历史分位计算窗口（默认 5y→3y）· 与同花顺估值分析对齐
PE_PCTL_WINDOW_DAYS = 3 * 365
PE_PCTL_WINDOW_LABEL = '3y'
...
def calc_stock_percentile(code, val):
    ...
    if pe_ttm is None:
        ...
        if len(history) >= WIN_HISTORY_MIN:
            # ★ commit 4.54：3y 窗口过滤（与正常股分支一致）
            from datetime import datetime, timedelta
            _window_start_loss = (datetime.today() - timedelta(days=PE_PCTL_WINDOW_DAYS)).strftime('%Y-%m-%d')
            _windowed_history = [h for h in history if h.get('date', '') >= _window_start_loss and h.get('pe') is not None]
            if len(_windowed_history) == 0:
                _windowed_history = [h for h in history if h.get('pe') is not None]   # 降级：3y 内无有效 PE
            pes = np.array([h['pe'] for h in _windowed_history])
    ...
    # ★ commit 4.54：3y 窗口过滤（默认 5y→3y）·与同花顺估值分析对齐
    from datetime import datetime, timedelta
    _window_start = (datetime.today() - timedelta(days=PE_PCTL_WINDOW_DAYS)).strftime('%Y-%m-%d')
    _windowed_history = [h for h in history if h.get('date', '') >= _window_start and h.get('pe') is not None]
    if len(_windowed_history) == 0:
        _windowed_history = [h for h in history if h.get('pe') is not None]   # 降级：3y 内无有效 PE·用全量
    pes = np.array([h['pe'] for h in _windowed_history])
